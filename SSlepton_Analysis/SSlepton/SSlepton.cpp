void SSlepton(){
  ...
  std::string year[] = {"2016", "2017"};
  
  for (int i=0; i<2; i++){ 
 
  //Open root file 
  TFile *DATA = TFile::Open("/home/snuintern1/skflat/SKFlatOutput/Run2Legacy_v3/SSlepton/"+year[i]+"/DATA/SSlepton_SingleMuon.root");
  ...
  //Save as pdf file
  c1->SaveAs("./output/"+year[i]+"/medium/++_dimuon_DATA.pdf");
  c2->SaveAs("./output/"+year[i]+"/medium/--_dimuon_DATA.pdf");
  c3->SaveAs("./output/"+year[i]+"/medium/ratio_dimuon_DATA.pdf");
  c4->SaveAs("./output/"+year[i]+"/tight/++_dimuon_DATA.pdf");
  c5->SaveAs("./output/"+year[i]+"/tight/--_dimuon_DATA.pdf");
  c6->SaveAs("./output/"+year[i]+"/tight/ratio_dimuon_DATA.pdf");
  }
}
